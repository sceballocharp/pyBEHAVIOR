...
class BasilAcquisitionApp(tk.Tk):
    def __init__(self):
        ...
        self.sound_ao_task = None
        self.sound_do_task = None

        self._build_ui()
        self.generate_sequence(log=False)
        self.after(50, self._drain_plot_queue)
        self.log("pyBASIL_v3 ready.")
        if nidaqmx is None:
    ...
    def update_trial_duration(self):
        total = (
            ...
            + self.parse_float(self.sound_duration_s, 0)
    ...
    def write_parameters_dat(self):
        ...
        with open(path, "w", encoding="utf-8") as f:
            ...
            f.write(f"GoWeight={sequence_weights[0] if len(sequence_weights) > 0 else ''}\n")
            f.write(f"NoGoWeight={sequence_weights[1] if len(sequence_weights) > 1 else ''}\n")
            f.write(f"GoSoundId={sequence_values[0] if len(sequence_values) > 0 else ''}\n")
            f.write(f"NoGoSoundId={sequence_values[1] if len(sequence_values) > 1 else ''}\n")
    ...
    def create_trial(self, sound_id, trigger_time_s, threshold, iti):
        ...
        parameter_row = {
            ...
            "iti_s": iti,
            "iti_rand_min_s": self.iti_rand_min_s.get(),
            "iti_rand_max_s": self.iti_rand_max_s.get(),
            "sound_delay_s": self.parse_float(self.sound_delay_s, 0),
            "sound_duration_s": self.sound_duration_s.get(),
            "trial_duration_s": self.parse_float(self.trial_duration_s, 3),
            "response_window_s": self.parse_float(self.response_window_s, 2),
            "reward_delay_s": self.parse_float(self.reward_delay_s, 0),
            "reward_duration_ms": self.parse_float(self.pulse_ms, 50),
            "punish_interval": self.punish_interval.get(),
            "reward_go": self.reward_go.get(),
            "punish_no_go_fa": self.punish_no_go_fa.get(),
            "min_lick_count": self.min_lick_count.get(),
            "lick_threshold": self.lick_threshold.get(),
            "play_sound": int(self.play_sound_on_crossing.get()),
            "trigger_output": int(self.trigger_output_on_crossing.get()),
            "Block": "",
        }
    ...
    def write_trial_log(self):
        if self.trial_log_path:
            with open(self.trial_log_path, "w", newline="", encoding="utf-8") as f:
                fieldnames = list(self.trial_rows[0].keys()) if self.trial_rows else []
    ...
    def get_parameter_block_label(self, parameter_row=None):
        ...
        signature = tuple(
            ...
            if key not in ("Block", "trial", "timestamp", "sound_id", "trigger_time_s")
        )
    ...
    def write_parameters_csv(self):
        if self.parameters_log_path:
            with open(self.parameters_log_path, "w", newline="", encoding="utf-8") as f:
                fieldnames = list(self.parameter_rows[0].keys()) if self.parameter_rows else []
                writer = csv.DictWriter(f, fieldnames=fieldnames)
                if fieldnames:
                    writer.writeheader()
                    writer.writerows(self.parameter_rows)
    ...
    def evaluate_active_trial(self, sample_time_s):
        ...
        if is_go and total_s >= hit_threshold_s and not row["HIT"]:
            row["HIT"] = 1
            row["MISS"] = 0
            row["CR"] = 0
            row["FA"] = 0
    ...
    def evaluate_active_lick_trial(self, row):
        ...
        is_go = row["TrialType"].endswith("GO")
        is_nogo = row["TrialType"].endswith("noGo")
        if is_go and self.active_lick_count >= min_lick_count and not row["HIT"]:
            row["HIT"] = 1
            row["MISS"] = 0
            row["CR"] = 0
            row["FA"] = 0
    ...
    def build_sound_timeseries(self):
        if not self.full_sound_outputs:
            if np is not None:
